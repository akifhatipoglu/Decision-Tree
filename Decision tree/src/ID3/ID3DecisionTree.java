...
public class ID3DecisionTree {
	...
	public ID3DecisionTree(SampleCollection sampleColl) {
		...
		//System.exit(0);
		rootNode = ID3(sampleColl.FEATURES.subFeatureList,
	...
	private ID3DecisionTreeNode ID3(List<String> FeatureList,ArrayList<String> sinifEtiketleri, List<Sample> sampleList) {
		
		ID3DecisionTreeNode geciciNode = null; 
		// null kalkacak
		...
		if (FeatureList.isEmpty()) {
			geciciNode = new ID3DecisionTreeNode(true, getProminentClass(sampleList));
			return geciciNode;
		}
		...
		if(bestFeatureIndex == ONBUDAMA){
			geciciNode = new ID3DecisionTreeNode(true, getOnBuda(sampleList));
			return geciciNode;
		}
		...
		geciciNode = new ID3DecisionTreeNode(FeatureList.get(bestFeatureIndex),bestOfAttrVals.size());
		...
		for (String attrVal : bestOfAttrVals) {
			...
			ID3DecisionTreeNode childNode = ID3(new ArrayList<String>(subFeatures), sinifEtiketleri, subset);
